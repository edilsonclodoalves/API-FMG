...
class FortiManager:
    ...
    def __init__(self, host, username="admin", password="admin", adom="root", protocol="https", verify=False, #Add verify false
                 proxies={}):
    ...
    def commit_changes(self):
        """
        Commit changes in the current Adom in FortiManager.
        Adom has to be in workspace mode and locked before committing.
        :return: Response of status code (0=success) with data in JSON Format
        """
        payload = \
            {
                ...
                "params": 
                    [
                        {
                             "url": f"/dvmdb/adom/{self.adom}/workspace/commit"
                        }
                    ],
            }

        response = self.custom_api(payload)
        return response
    ...
    def add_dynamic_object(self, name, device, subnet=list, comment=None, revision_name=None):
        ...
        payload = {
            "method": "add",
            "params": [{"url": f"pm/config/adom/{self.adom}/obj/firewall/address/{name}/dynamic_mapping",
